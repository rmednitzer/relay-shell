...
Closed in the 2026-08-04 backlog follow-up (`audit/2026-08-04-backlog-followup.md`):

| ID | Sev | Title | Resolution |
|----|-----|-------|------------|
| EVID-1 | MED | Audit-evidence verifier ordered segments by filesystem mtime (logrotate `delaycompress` inverts it) and read the live `audit.jsonl` `errors="strict"`, so a torn write / mtime inversion marked the run `valid:false` → `relay-audit-evidence-sync` silently suppressed the off-host copy (anti-tail-truncation control, ADR 0007) | **Closed** (follow-up PR). Two-phase restructure: `verify_segment()` verifies each segment independently, then phase 2 orders by the first record's `ts` (chain-authoritative chronological order, immune to mtime inversion; robust across approved genesis resets where `first_seq`=0 recurs) for the seam checks. A parse failure on the *last* non-empty line of the live segment is deferred and tolerated as a torn append (`torn_tails` counter for visibility); anywhere else, or on any rotated segment, stays a hard error. 4 verifier tests in `tests/test_evidence_deploy.py` (out-of-mtime-order verifies clean; torn final line stays valid; malformed interior line fails; torn tail on a rotated segment fails). |
| PERF-4 | info→MED | Single-host exec buffered unbounded output before truncation: `shell_exec`/`shell_script` (`communicate()`) and `ssh_exec` (`SshPool.run` with `max_output_bytes` unset → `cap is None`) — a memory DoS reachable by a Tier-1 command `guarded` mode permits | **Closed** (follow-up PR). `shelltools._drive` now drains stdout/stderr to EOF keeping at most `output_cap` bytes (a shared budget), feeding stdin concurrently so a large stdin cannot deadlock; the server passes `max_output_hard`. `ssh_exec` passes `max_output_bytes=max_output_hard` to use `SshPool.run`'s bounded drain. Child runs to completion (exit code + true count preserved); returned output byte-identical after truncation. 7 tests in `tests/test_shell.py` (bounded buffer, exit-code preserved, stdin works, large-stdin no-deadlock, shared budget, unbounded default, `run_script`). Upgraded from info to MED on review: it crosses the `guarded` boundary. |
