...
| [0007](0007-audit-hash-chain.md) | Tamper-evident audit log via per-record hash chaining | Accepted | 2026-06-01 | An opt-in (`RELAY_SHELL_AUDIT_CHAIN`, default off), additive per-record hash chain (`seq`/`prev`/`chain`) that makes edits, insertions, reorders, and interior deletions of the on-disk audit log detectable by recomputation (head-truncation via the genesis anchor / `--require-genesis`; tail-truncation via the off-host copy) — closing the integrity gap left by `chattr +a` + off-host shipping against the ADR 0002 residual-risk attacker. `jsonl` only; verified offline by `relay-shell --verify-audit` (a CLI verb, not an MCP tool). |
